...
1. **Run `python3 scripts/check_params.py` and `python3 scripts/geometry_check.py` first.** A `[FAIL]` means parts physically won't fit. This gates everything else. (`performance_check.py` is the third gate but needs regenerated STLs for its mass model — regen_all runs it in the right order.)
...
- **Horizontal rod holes go through `lib/bores.scad` (teardrop)** and parts are modeled in their print orientation — a plain horizontal cylinder bore is a finding. Exception: the hub shell's arm seats, the die jig's groove, and the hub clamshell variant's grooves and bolt bores are plain rounds, because all of them print their open faces up (seats and channels open upward, bolt bores vertical).
...
- **The swing-stop mechanism is load-bearing design**: each vane must swing vane_swing_deg freely and hit a hard stop (the end cap's wedge in the sleeve's top notch; the cap is the ONLY stop and clamps at any angle; below, the sleeve rides a bought PTFE washer on the bracket's flat top). Changes to sleeve, cap, or wedge must keep `geometry_check.py`'s wedge checks meaningful — never widen the notch to "fix" a stop collision, and one wedge takes the whole clack, so its shear gate in `performance_check.py` sizes stop_wedge_deg. The flag's cut-away top inner corner (vane_shoulder_w) must keep the cap's bolt hardware clear at EVERY cap angle (gated), because the stop angle is a field adjustment. The tri variant uses the dual's vane assembly verbatim (same bracket, washer, vane and cap), so there are no tri-only stop parts.
- **Assembly is slide-and-clamp, nothing is drilled**: the hub is a washer-clamped shell sandwich on the die-threaded shaft top and the tip brackets are clamshells (hub_clamp_gap and bracket_clamp_gap must stay nonzero so preload lands on the rods), every other rod joint is a WIDE dual-bolt slit clamp (geometry_check gates the grip length). A single set screw, a narrow friction ring, or a feature requiring a hole in a rod is a finding. No feature may require a rod cut to better than a couple of millimeters (the cap bore leaves slack for this).
